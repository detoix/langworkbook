...
const createPool = (connectionString) => {
  ...
  pool.getMyExercises = student => pool
    .query("SELECT exercises.id, exercises.author, exercises.tags, exercises.data, actions.timestamp, actions.result->>'answer' as answer FROM actions \
      LEFT JOIN exercises ON actions.exercise = exercises.id AND actions.student = $1 \
      WHERE actions.student = $1", [student])
    .then(data => data.rows)
